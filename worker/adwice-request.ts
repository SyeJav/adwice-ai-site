...
export interface AdwiceEnv { ADWICE_API_BASE_URL?: string; ADWICE_SMTP_PASSWORD?: string }
...
function validate(body: Record<string, unknown>): FieldErrors {
  ...
  if (typeof body.email === "string" && body.email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email.trim())) errors.email = ["Enter a valid email address."];
  if (typeof body.url === "string" && body.url.trim()) {
    try { const url = new URL(body.url.trim()); if (!["http:", "https:"].includes(url.protocol)) throw new Error(); }
...
export async function handleAdwiceRequest(request: Request, env: AdwiceEnv): Promise<Response> {
  if (request.method !== "POST") return new Response(null, { status: 405, headers: { Allow: "POST" } });
  let input: Record<string, unknown>;
  try { input = await request.json() as Record<string, unknown>; }
  catch { return json({ status: "fail", message: "Invalid JSON request.", code: 400, data: {} }, 400); }
  const errors = validate(input);
  if (Object.keys(errors).length) return json({ status: "fail", message: "Please check the highlighted fields.", code: 422, data: errors }, 422);
  const payload = {
    name: (input.name as string).trim(), email: (input.email as string).trim(), url: (input.url as string).trim(),
